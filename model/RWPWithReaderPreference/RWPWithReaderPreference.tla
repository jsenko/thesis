----------------------- MODULE RWPWithReaderPreference -------------------------
EXTENDS Naturals\*, Semaphore
...
mutexInstance == INSTANCE Semaphore WITH permitCount <- 1
wInstance == INSTANCE Semaphore WITH permitCount <- 1
...
s01(self) == /\ pc[self] = "s01"
             /\ \E x \in BOOLEAN:
                  /\ s_' = [s_ EXCEPT ![self] = x]
                  /\ mutexInstance!TryWait(x, mutex, mutex')
